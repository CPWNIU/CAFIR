...
from datasets.occ_reid import occ_reid
from datasets.partial_reid import Partial_reid
from datasets.partial_ilids import Paritial_ilids
from datasets.p_dukemtmc import P_DukeMTMC
from datasets.p_ethz import P_ETHZ
...
def do_train(cfg,
    ...
    if cfg.MODEL.ABL =='onlyMULTI':
        ...
        for epoch in range(1, epochs + 1):
            ...
            scheduler.step(epoch)
            model.train()
            for n_iter, (img, img2, img3, vid, target_cam, target_view) in enumerate(train_loader):
                optimizer.zero_grad()
                optimizer_center.zero_grad()
                ...
                with amp.autocast(enabled=True):
                    ...
                    if cfg.MODEL.ARC == 'OAFR':
                        score, feat, occ_feat, occ_lable_gt_2d, occ_lable_gt, occ_pre_label, local_feat, rand_index, from_copy_tensor_label = model(
                            img, img2, img3, cfg.MODEL.OCC_AUG, cfg.SOLVER.PERSON_OCC_PRO, target, cam_label=target_cam,
                        ...
                        score, feat = model(img, img2, img3, cfg.MODEL.OCC_AUG, cfg.SOLVER.PERSON_OCC_PRO, target,
                                            cam_label=target_cam, view_label=target_view)
                        loss = loss_fn(score, feat, target, target_cam)
                scaler.scale(loss).backward()
                scaler.step(optimizer)
                ...
                if (n_iter + 1) % log_period == 0:
                    logger.info(
                        "Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, Acc: {:.3f},Loss_pid: {:.3f},Loss_occPredict: {:.3f} Base Lr: {:.2e}"
                        .format(epoch, (n_iter + 1), len(train_loader),
                                loss_meter.avg, acc_meter.avg, loss_meter1.avg, loss_meter2.avg,
            ...
            if epoch % checkpoint_period == 0:
                if cfg.MODEL.DIST_TRAIN:
                    if dist.get_rank() == 0:
                        torch.save(model.state_dict(),
                                   os.path.join(cfg.OUTPUT_DIR, cfg.MODEL.NAME + datetime.datetime.now().strftime(
                                       '%m%d%H%M') + '_{}.pth'.format(epoch)))
                else:
                    torch.save(model.state_dict(),
                               os.path.join(cfg.OUTPUT_DIR, cfg.MODEL.NAME + datetime.datetime.now().strftime(
                                   '%m%d%H%M') + '_{}.pth'.format(epoch)))
            ...
            if epoch % eval_period == 0:
                if cfg.MODEL.DIST_TRAIN:
                    if dist.get_rank() == 0:
                        model.eval()
                        for n_iter, (img, img2, img3, vid, camid, camids, target_view, _) in enumerate(val_loader):
                            with torch.no_grad():
                                img = img.to(device)
                                img2 = img2.to(device)
                                img3 = img3.to(device)
                                ...
                                if cfg.MODEL.ARC == 'OAFR':
                                    globla_feats, local_feat_h, wei_h, co_h_pro = model(img, img2, img3,
                                                                                        cfg.SOLVER.PERSON_OCC_PRO,
                                                                                        cam_label=camids,
                                                                                        view_label=target_view)
                                    evaluator.update((globla_feats, vid, camid))
                                    evaluator.update_w(
                                        (globla_feats, local_feat_h, wei_h, local_feat_h, co_h_pro, vid, camid))
                                else:
                                    feat = model(img, img2, img3, cam_label=camids, view_label=target_view)
                                    evaluator.update((feat, vid, camid))
                        if cfg.MODEL.ARC == 'OAFR':
                            cmc, mAP, _, _, _, _, _ = evaluator.compute()
                    ...
                    if cfg.MODEL.ARC == 'OAFR':
                        ...
                        for r in [1, 5, 10]:
                            logger.info(
                                "local_recovery_CMC curve, Rank-{:<3}:{:.1%}".format(r, cmc_local_recovery[r - 1]))
...
def do_inference(cfg,
    ...
    for n_iter, (img, img2, img3, pid, camid, camids, target_view, imgpath) in enumerate(val_loader):
        with torch.no_grad():
            img = img.to(device)
            img2 = img2.to(device)
            img3 = img3.to(device)
            ...
            if cfg.MODEL.ARC == 'OAFR':
                globla_feats, local_feat_h, wei_h, co_h_pro = model(img, img2, img3, cfg.SOLVER.PERSON_OCC_PRO,
                                                                    cam_label=camids,
                                                                    view_label=target_view)
